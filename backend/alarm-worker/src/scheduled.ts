...
export interface ScheduledStats extends LiveActivityStats {
  ...
  /**
   * #2651 — `evaluateAndMaybeFireBoardingPrompt`(GPS 9단 leg-1 경로)가
   * `trip.promptOptIn !== true && trip.infoModeEnabled !== true`(안내 시작도 안 눌렀고 응답/직접
   * 탭 이력도 없는, 목적지만 설정한 trip)로 즉시 return한 누적 횟수. 이전엔 이 경로에 opt-in
   * 게이트가 전혀 없어 등록만으로 지상 프롬프트가 발사됐다(결정 모델 위반) — 0이 아닌 것이
   * 정상(무의향 trip이 실제로 차단되고 있다는 신호). GPS-free 경로
   * (`maybeFireOriginBoardingPromptGpsFree`)의 동일 게이트와 짝.
   *
   * #2651 (PR #2772 리뷰) — `infoModeEnabled`도 OR로 추가돼, 이미 응답/직접 탭으로 명시 의향을
   * 표명한 trip은 "안내 시작"을 다시 누르지 않아도 이 skip에 걸리지 않는다(ADR-014 동급 보장).
   */
...
export function createEmptyScheduledStats(now: number): ScheduledStats {
  return {
    ...
    boardingAnchorLegStreakPending: 0,
...
export async function runScheduled(env: Env, deps: ScheduledDeps): Promise<ScheduledStats> {
  ...
  for (const trip of trips) {
    ...
    if (!isBoardingLockActive(trip, now)) {
      ...
      // cycle은 여기서 종료한다 — 다음 cycle부터 위 `isBoardingLockActive` 판정이 true가 되어
      // 정상 `runTrainCodeTracking` 경로(매역 push)로 진입한다. ambiguous/none은 그대로
...
export async function fireArvlCdStationPush(
  ...
  // 아래에서는 ssotForFireGate를 그대로 재사용한다.
...
interface FireVanishFallbackStationPushInputs {
   ...
   *
   * #2764 (게이트 전수감사 A) — 과거 이 필드로 origin별 dedup key를 분리해 "release 후 lock
   * 재부착(swap 성공)으로 같은 station에서 advance 경로가 추가 발사"를 허용하려 했으나, #2571
   * station 단위 dedup(stationPassedFiredKey, 경로 무관)이 이미 그 시나리오를 무효화했다 —
   * origin별 키는 도달 불가로 판정돼 삭제됐다(PR #2764).
   */
...
export async function maybeFireOriginBoardingPromptGpsFree(
  ...
  //
  // #2651 (PR #2772 리뷰) — `promptOptIn === true || trip.infoModeEnabled === true`로 OR.
  // 이미 응답/직접 탭으로 명시 의향(`infoModeEnabled`)을 표명한 trip이 lock 해제 후
  // lockMissing으로 돌아왔을 때도 재프롬프트를 받아야 한다(ADR-014 동급 보장) — 위 GPS 9단
  // 게이트와 동일한 OR 정합.
  if (trip.promptOptIn !== true && trip.infoModeEnabled !== true) return;
