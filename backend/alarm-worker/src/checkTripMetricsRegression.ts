 ...
 *
 * ## 공통 회귀 신호 (SSoT — 2026-09-24 결정 코멘트, `tripMetricsRegressionScan.ts`(옵션 A)와
 * 동일 조건)
 *
 *   ended_at IS NOT NULL
 *   AND (lock_attached = 1 OR boarding_prompt_responded = 1)
 *   AND fired_count = 0
 *   AND started_at > now - 1day
 *
 ...
 * 스크립트 셸이 `Date.now() - 24*60*60*1000`을 넘긴다).
 */
