...
describe('advanceTripPosition — 6단 게이트 양방향 시나리오 (acceptance 매핑)', () => {
  ...
  const scenarios: Array<{
    ...
    {
      name: 'N6b (#2623 회귀방어) lock + unknown env + arvlcd 확증 없는 evidence(position-train) + lockAttachable=false → blocked(env-consensus-fail) (bypass는 arvlcd-confirmed-train 전용)',
      motion: 'moving',
      userIntent: false,
      hasLock: true,
      env: 'unknown',
