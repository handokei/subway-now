...
describe('POST /trips/:token/boarding-confirm (#2527)', () => {
  ...
  function positionEntry(overrides: Record<string, unknown> = {}): Record<string, unknown> {
    return {
      ...
      trainSttus: 1,
      updnLine: '1',
      recptnDt: recptnDtFor(CREATED),
      lastRecptnDt: recptnDtFor(CREATED).slice(0, 10).replace(/-/g, ''),
      ...overrides,
    };
  }
            ...
            realtimePositionList: [
              {
                ...
                trainSttus: 1,
                updnLine: '1',
                recptnDt: recptnDtFor(Date.now()),
                lastRecptnDt: recptnDtFor(Date.now()).slice(0, 10).replace(/-/g, ''),
              },
            ],
          }),
          { status: 200 },
