...
export interface Trip {
   ...
   * backend side에서 한 번 더 게이트한다. #2571 이후의 station-passed 단일 dedup 마커
   * (`stationPassedFiredKey`, 경로 무관)는 같은 (token, trainCode, station) 조합 재발사만
   * 막아 cross-station은 통과시킨다 — 이 필드는 그 cross-station 갭을 메운다. (구)per-arvlCd
   * dedup(`arvlCdFireKey`)은 도달불가 확증 후 PR #2764로 삭제됐다.
   */
