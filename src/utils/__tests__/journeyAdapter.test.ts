...
describe('journeyDisplayToStops', () => {
  it('should convert a direct route (single segment)', () => {
    ...
    expect(stops).toEqual([
      { station: '강남', line: '2', mark: 'filled' },
      {
        ...
        mark: 'dest',
        note: '도착',
        arrivalContext: { line: '2', fromName: '강남', toName: '역삼' },
      },
    ]);
  });
  ...
  it('should convert a transfer route (two segments)', () => {
    ...
    expect(stops).toEqual([
      { station: '효창공원앞', line: '6', mark: 'filled' },
      {
        ...
        mark: 'transfer',
        note: '환승',
        arrivalContext: { line: '6', fromName: '효창공원앞', toName: '공덕' },
      },
      {
        station: '여의나루',
        line: '5',
        ...
        mark: 'dest',
        note: '도착',
        arrivalContext: { line: '5', fromName: '공덕', toName: '여의나루' },
      },
    ]);
  });
  ...
  it('should convert a multi-transfer route (three segments)', () => {
    ...
    expect(stops[1]).toEqual({
      ...
      mark: 'transfer',
      note: '환승',
      arrivalContext: { line: '1', fromName: '서울역', toName: '시청' },
    });
    expect(stops[2]).toEqual({
      ...
      mark: 'transfer',
      note: '환승',
      arrivalContext: { line: '2', fromName: '시청', toName: '을지로3가' },
    });
    expect(stops[3]).toEqual({
      ...
      mark: 'dest',
      note: '도착',
      arrivalContext: { line: '3', fromName: '을지로3가', toName: '경복궁' },
    });
  });
  ...
  it('환승역이 곧 목적지이면 도착 노드를 환승 노드에 흡수한다', () => {
    ...
    expect(stops[1]).toEqual({
      ...
      mark: 'transfer',
      note: '환승',
      arrivalContext: { line: '2', fromName: '삼성', toName: '건대입구' },
    });
    ...
    expect(stops[2]).toEqual({
      ...
      mark: 'dest',
      note: '환승 → 도착',
      arrivalContext: { line: '7', fromName: '건대입구', toName: '군자' },
    });
  });
});
