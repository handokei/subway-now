...
describe('fetchArrivalInfo', () => {
  ...
  describe('schedule fallback', () => {
    ...
    it('역명은 알지만 headway 데이터 없는 노선이면 MOCK_ARRIVALS 최후 fallback', async () => {
      ...
      jest.isolateModules(() => {
        jest.doMock('../../../alarm/utils/scheduleFallback', () => {
          const actual = jest.requireActual('../../../alarm/utils/scheduleFallback');
  ...
  describe('line 매핑 (#663)', () => {
    ...
    it('subwayId가 응답마다 다르면 같은 statnNm 환승역 응답을 row별로 정확히 분리한다', async () => {
      ...
      expect(result.up.map((r) => r.line)).toEqual(['7', 'gyeongui']);
    });

    it('subwayId 누락 또는 매핑 실패 시 lineHint로 채워진다', async () => {
      mockRealtimeArrivalList([
        { trainLineNm: '강남행', barvlDt: 60, btrainNo: 'T1', updnLine: '상행' },
        { trainLineNm: '강남행', barvlDt: 90, btrainNo: 'T2', updnLine: '상행', subwayId: '9999' },
      ]);
      const result = await fetchArrivalInfo('강남', { lineHint: '2' });
      expect(result.up.every((r) => r.line === '2')).toBe(true);
    });

    it('subwayId 누락 + lineHint 없으면 row가 drop된다', async () => {
      mockRealtimeArrivalList([
      ...
      const result = await fetchArrivalInfo('강남');
      expect(result.up).toHaveLength(1);
      expect(result.up[0]).toMatchObject({ trainCode: 'T2', line: '1' });
    });

    it('subwayId가 있으면 lineHint보다 우선', async () => {
      mockRealtimeArrivalList([
        { trainLineNm: '7호선행', barvlDt: 60, btrainNo: 'T-7', updnLine: '상행', subwayId: '1007' },
      ]);
    ...
    it('모든 row가 line 식별 실패면 schedule fallback', async () => {
      mockRealtimeArrivalList([
