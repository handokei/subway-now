/**
 * 모든 알람/알림 발사 경로 통합 정합성 게이트 — frontend mirror (#1389).
 *
 * device가 발사/수신하는 모든 push (silent push 수신 처리 / boarding-prompt /
 * pre-scheduled `bl:`/`tba:` fire / FG fire / Live Activity render) 가
 * device의 현재 신호 (currentStationName / motion / WiFi) 와 모순되지
 * 않도록 발사 직전 검사하는 순수 함수.
 *
 * 정책 원칙:
 ...
 *
 * backend mirror: backend/alarm-worker/src/pushConsistency.ts
